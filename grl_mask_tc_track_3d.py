...
def mask_tc_track_3d(track_file, rmax, var, lon_tmp, lat, t0, t1):

    # Input dimensions
    # nt,nz,nx1,nx2 = var.shape
    nt,nx1,nx2 = var.shape
    ...
    var_mask = np.ma.masked_where(radius3d > rmax, var, copy=True)
    # print(np.ma.count_masked(var_mask))

    # Mask out domain edges, within 0.5*r_max from boundaries
    edge = 1 # deg
    npts = int(np.rint(edge/(lon[0,1]-lon[0,0])))
    # var_mask.mask[:,:,0:npts,:] = True
    var_mask.mask[:,0:npts,:] = True # removed one dimension
    var_mask.mask[:,:,0:npts] = True # removed one dimension
    var_mask.mask[:,-npts:,:] = True # removed one dimension
    var_mask.mask[:,:,-npts:] = True # removed one dimension
    # print(np.ma.count_masked(var_mask))
